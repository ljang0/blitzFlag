...
predictors = c("xDiff", "yDiff", "lineDiffX", "lineDiffY", "lbDiffX", "lbDiffY", 'dir', 'o')
...
new_feature_names <- c("Difference in X Coordinate from AVG D-Line", "Difference in X Coordinate from Center", "Difference in Y Coordinate from Center", "Player Direction", "Difference in Y Coordinate from AVG D-Line", 
                        
                       "Player Orientation", "Difference in Y Coordinate from AVG LB", "Difference in X Coordinate from AVG LB")
if(length(new_feature_names) == nrow(importance_matrix)) {
  # Assign new feature names
  importance_matrix$Feature <- new_feature_names
} else {
  stop("Number of new feature names does not match the number of features.")
}
# Optionally, you can also visualize the importance
xgb.plot.importance(importance_matrix)
ggplot(importance_matrix, aes(x = reorder(Feature, Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  theme_minimal() +
  labs(title = "Top Variable Importance", x = "Feature", y = "Importance")
```
...
print(metrics_by_group)
pander::pander(metrics_by_group)
```
...
```{r}
majority_labels <- linebackersDF %>%
  group_by(pff_positionLinedUp) %>%
  summarise(MajorityLabel = names(which.max(table(label))),
            .groups = 'drop')
linebackersDF <- linebackersDF %>%
  left_join(majority_labels, by = "pff_positionLinedUp") %>%
  mutate(MajorityClassPrediction = as.integer(MajorityLabel)) %>%
  select(-MajorityLabel) # Remove the MajorityLabel column after adding Prediction

# Assuming the actual label column in zeroDefenseFrameAll is named `Label`

# Step 2: Calculate accuracy
accuracy <- sum(linebackersDF$label == linebackersDF$MajorityClassPrediction) / nrow(linebackersDF)
conf_matrix <- confusionMatrix(as.factor(linebackersDF$MajorityClassPrediction), as.factor(linebackersDF$label))
print(conf_matrix)
...
linebackersDF <- linebackersDF %>%
  mutate(CorrectMajorityPrediction = label == MajorityClassPrediction)
metrics_by_group <- linebackersDF %>%
  group_by(Position) %>%
  summarise(
    Accuracy = mean(CorrectMajorityPrediction),
    Precision = sum(label == 1 & MajorityClassPrediction == 1) / sum(MajorityClassPrediction == 1),
...
#colnames(linebackersDF)[colnames(linebackersDF) == "pff_positionLinedUp_str"] <- "Position"
...
```{r}
mlmCoordsLB  <- glmer(label ~ 1 + xDiff + yDiff + pff_positionLinedUp + (1 | nflId) + (1 | personnelD) + (1 | team), data = train_data, family = binomial)
### FOR RANDOM INTERCEPTS:

# Print the summary of the model
summary(mlmCoordsLB)

test_data$predicted_probabilities <- predict(mlmCoordsLB, newdata = test_data, type = "response", allow.new.levels = TRUE)
...
pander(metrics_by_group)

```


#### decision tree for Assignment 2
...
train_data <- linebackersDF[linebackersDF$gameId %in% train_gameids, ]
test_data <- linebackersDF[!(linebackersDF$gameId %in% train_gameids), ]
tree_model <- rpart(label ~ pff_positionLinedUp + xDiff + yDiff + lineDiffX + lineDiffY + lbDiffY + lbDiffX, data = train_data, method = "class")
predictions <- predict(tree_model, test_data, type = "class")
test_data$predictions = predictions
conf_matrix <- confusionMatrix(predictions, test_data$label)
# Display the confusion matrix
...
custom_names <- c("pff_positionLinedUp" = "Player Position", "xDiff" = "X Distance from Center", "yDiff" = "Y Distance from Center", 
                  "lineDiffY" = "Y Distance from Average D-Lineman", "lineDiffX" = "X Distance from Average D-Lineman", "lbDiffY" = "Y Distance from Average Linebacker", "lbDiffX" = "X Distance from Average Linebacker")

# Apply custom names to the sorted importance vector
names(importance_sorted) <- custom_names[names(importance_sorted)]
